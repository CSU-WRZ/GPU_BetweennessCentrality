...
TreeDecomp::TreeDecomp(const char *filein, const char *fileout) : TreeDecomp()
{
    ...
    for (int i = 0; i <= G_.n(); ++i) {
        ...
        my_edge_count_.push_back(vecLLtmp);
    }
    ...
    for (int i = 0; i <= G_.n(); ++i) {
        for (auto v : G_.E_[i]) {
            ...
            int u = i;
            my_C_[u].insert(std::make_pair(v.first, 1));
            my_C_[v.first].insert(std::make_pair(u, 1));
        }
    }
}
...
void TreeDecomp::Reduce()
{
    ...
    while (!deg.empty()) {
        ...
        vector<count_t> my_tmp_edge_count; // hop_number;
        my_tmp_edge_count.clear();
        ...
        for (auto y : G_.E_[x]) {
            if (exist[y.first]) {
                ...
                my_tmp_edge_count.push_back(my_C_[x][y.first]);
            }
        }
                ...
                if (pu != pv) {
                    ...
                    if (G_.isEdgeExist(u, v)) {
                        ...
                        if (G_.E_[u][v] == newlength) {
                            UpdateCount(u, v, C_[u][v] + tmp_edge_count[pu] * tmp_edge_count[pv]);
                            if (my_tmp_edge_count[pu] + my_tmp_edge_count[pv] > my_C_[u][v]) {
                                my_C_[u][v] = my_tmp_edge_count[pu] + my_tmp_edge_count[pv];
                                my_C_[v][u] = my_tmp_edge_count[pu] + my_tmp_edge_count[pv];
                            }
                        }
                        if (G_.E_[u][v] > newlength) {
                            G_.E_[u][v] = newlength;
                        ...
                        my_C_[u].insert(std::make_pair(v, my_tmp_edge_count[pu] + my_tmp_edge_count[pv]));
                        my_C_[v].insert(std::make_pair(u, my_tmp_edge_count[pu] + my_tmp_edge_count[pv]));
        ...
        if (tmp_neighbor.size() > tree_width_)
            tree_width_ = tmp_neighbor.size();
        neighbor_[x] = tmp_neighbor;
        length_[x] = tmp_length;
        edge_count_[x] = tmp_edge_count;
        my_edge_count_[x] = my_tmp_edge_count;
    }
    free(DD);
    free(tmp_DD);
    free(DD2);
...
void TreeDecomp::MakeTree()
{
    ...
    for (int len = delete_vertex_order_.size() - 1; len >= 0; --len) {
        ...
        if (len == G_.n() - 1) {
            ...
            if (nod.height > tree_height_)
                tree_height_ = nod.height;
        }
        tree_.push_back(nod);
    }
    // free(rank_);
...
{
    MakeRMQIndex();
    // PrintIndex();--------------------------------- comment here

    MakeCountIndex();
}

void TreeDecomp::EulerTour(int p, int height)
{ // p: position in tree_
...
void TreeDecomp::PrintIndex()
{
    ...
    x = euler_tour_.size();
    fwrite(&x, SIZEOFINT, 1, fout_);
    for (int i = 0; i < rmq_index_.size(); i++)
        PrintIntVector(rmq_index_[i]);
    // rootDistance
    ...
    for (int i = 0; i < tree_.size(); i++) {
        ...
        for (int j = 0; j < t; j++)
            fwrite(&tree_[i].child[j], SIZEOFINT, 1, fout_);
    }
}
...
void TreeDecomp::makeIndexDFS(int p, vector<int> &list, int *toList)
{
    ...
    tree_[p].my_cnt.resize(list.size());

    for (int i = 0; i < tree_[p].vertices.size(); i++) {
        int j;
        for (j = 0; j < list.size(); j++)
            if (list[j] == tree_[p].vertices[i])
                break;
        tree_[p].pos[i] = j;
    }
    ...
    for (int i = 0; i < tree_[p].vertices.size(); i++) {
        ...
        if (tree_[p].dis[toList[tree_[p].vertices[i]]] > tree_[p].vertices_length[i]) {
            tree_[p].dis[toList[tree_[p].vertices[i]]] = tree_[p].vertices_length[i];
            tree_[p].cnt[toList[tree_[p].vertices[i]]] = edge_count_[tree_[p].vertex][i];
            tree_[p].my_cnt[toList[tree_[p].vertices[i]]] = my_edge_count_[tree_[p].vertex][i];
        }
    }
    ...
    for (int i = 0; i < tree_[p].vertices.size(); ++i) {
        ...
        for (int j = 0; j < list.size(); j++) {
            ...
            if (k < j) {
                ...
                my_route_new = my_edge_count_[tree_[p].vertex][i] + tree_[rank_[x]].my_cnt[j];
            }
...
int TreeDecomp::DistanceCountQuery(int p, int q, count_t &count)
{
    ...
    if (lca == x || lca == y) {
        ...
        my_count = tree_[y].my_cnt[tree_[x].pos[tree_[x].pos.size() - 1]];
    ...
    vector<count_t> &my_cx = tree_[x].my_cnt, &my_cy = tree_[y].my_cnt;
    for (int i = 0; i < ps; i++) {
        int tmp = dx[p2[i]] + dy[p2[i]];
        if (tmp_dis > tmp) {
            tmp_dis = tmp;
            cnt = cx[p2[i]] * cy[p2[i]];
            my_cnt = my_cx[p2[i]] + my_cy[p2[i]];
        } else if (tmp_dis == tmp) {
            cnt += cx[p2[i]] * cy[p2[i]];
            if (my_cx[p2[i]] + my_cy[p2[i]] > my_cnt) {
                my_cnt = my_cx[p2[i]] + my_cy[p2[i]];
            }
        }
    }
    ...
    // printf("p=%d q=%d my_count=%d\n", p, q, my_count);
    // cout << "p=" << p << " q=" << q << " my_count=" << my_count << endl;

    return my_count; // return dis;
}
...
void TreeQuery::ReadIndex(const char *index_file)
{
    ...
    if (rank_ == NULL)
        rank_ = (int *)malloc(SIZEOFINT * (n_ + 1));
    if (rank_ == NULL) {
        printf("rank_ == NULL");
        return;
    }
...
// read pos_size_, pos_, dis_;
#ifndef OMIT_POS_LIST_
    if (pos_size_ == NULL)
        pos_size_ = (int *)malloc(SIZEOFINT * tree_size_);
    if (pos_size_ == NULL) {
    ...
    if (pos_ == NULL)
        pos_ = (int **)malloc(sizeof(int *) * tree_size_);
    if (pos_ == NULL) {
        printf("pos_ == NULL");
        return;
    }
#else
    if (self_pos_ == NULL)
        self_pos_ = (int *)malloc(SIZEOFINT * tree_size_);
    if (self_pos_ == NULL) {
    ...
    if (dis_ == NULL)
        dis_ = (int **)malloc(sizeof(int *) * tree_size_);
    if (dis_ == NULL) {
        printf("dis_ == NULL");
        return;
    }
    ...
    if (cnt_ == NULL)
        cnt_ = (count_t **)malloc(sizeof(count_t *) * tree_size_);
    if (cnt_ == NULL) {
        printf("cnt_ == NULL");
        return;
    }
    ...
    for (int i = 0; i < tree_size_; ++i) {
        ...
        if (pos_size_[p] > tree_width_)
            tree_width_ = pos_size_[p];
#else
...
{
    if (LOG2 == NULL)
        LOG2 = (int *)malloc(sizeof(int) * (n_ * 2 + 10));
    if (LOGD == NULL)
        LOGD = (int *)malloc(sizeof(int) * (n_ * 2 + 10));
