...
class TreeDecomp {
 public:
  ...
  vector<vector<count_t> >edge_count_;  // record the number of shortest paths over short-cut edges.
                    // aligned with neighbor_, length_.
  vector<vector<count_t>>my_edge_count_;
  vector<map<int, count_t> > C_;  // count the number of shortest paths.

  vector<map<int, count_t> > my_C_;  // max hop number of shortest paths.
  ...
  vector<Node_> tree_;  // the decomposed tree structure
